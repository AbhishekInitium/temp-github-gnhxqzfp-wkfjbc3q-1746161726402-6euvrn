...
import { cleanSchemeForSave, cleanSchemeForView } from '../utils/schemeUtils';
...
export function SchemeDesigner() {
  ...
  const handleFileLoad = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          const loadedScheme = JSON.parse(e.target?.result as string);
          console.log('Loaded scheme:', loadedScheme);
          
          resetState();
          
          const validation = validateSchemeJson(loadedScheme);
          setValidationErrors(validation.errors);
          
          if (validation.valid) {
            const cleanedScheme = cleanSchemeForView(loadedScheme);
            setConfig(cleanedScheme);
            setSchemeName(cleanedScheme.name || '');
            setMode('view');
          }
        } catch (err) {
  ...
  const handleSave = () => {
    ...
    const updatedConfig = {
      ...
      name: schemeName,
      status: 'DRAFT',
      versionNumber: mode === 'new' ? 1 : (config.versionNumber || 0) + 1
    };

    const cleanedScheme = cleanSchemeForSave(updatedConfig);
    
    const filename = `${cleanedScheme.SchemeID}.json`;
    const dataStr = JSON.stringify(cleanedScheme, null, 2);
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setHasChanges(true);
                  setConfig(prev => ({ ...prev, name: value.slice(0, 10) }));
                }}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                setConfig(prev => ({ ...prev, quotaAmount: parseFloat(e.target.value) || 0 }));
                setHasChanges(true);
              }}
              disabled={mode === 'view'}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                setConfig(prev => ({ ...prev, effectiveFrom: e.target.value }));
                setHasChanges(true);
              }}
              disabled={mode === 'view'}
              className="w-full rounded-md border border-gray-300 shadow-inner px-3 py-2 focus:ring focus:outline-none disabled:bg-gray-50"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                setConfig(prev => ({ ...prev, effectiveTo: e.target.value }));
                setHasChanges(true);
              }}
              disabled={mode === 'view'}
              className="w-full rounded-md border border-gray-300 shadow-inner px-3 py-2 focus:ring focus:outline-none disabled:bg-gray-50"
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      disabled={mode === 'view'}
                      placeholder="e.g., Input_Sales.xlsx"
                      className="w-full rounded-md border border-gray-300 shadow-inner px-3 py-2 focus:ring focus:outline-none disabled:bg-gray-50"
                    />
                  </div>
